# -*- coding: utf-8 -*-
"""Connector module for Zotero (will be removed soon)."""
import sqlite3
from pathlib import Path


class ZoteroDBConnector:
    """This class manages the connections to Zotero's SQLite database."""
    ...
    def __init__(self, citekey, config):
        ...
        self.__fieldIDs = {"title": 110, "DOI": 26, "date": 14}
    ...
    def __get_creator_name(self):
        ...
        for creatorID in creatorIDs:
            result = self.__zotero.execute(
                'SELECT firstName, lastName from creators where creatorID="{}"'.format(
                    creatorID
                )
            )
    ...
    def get_field_values(self):
        """Retrieve field values from database."""
